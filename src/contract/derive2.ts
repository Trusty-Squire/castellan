...
export async function deriveV2(input: DeriveV2Input): Promise<DeriveV2Result> {
  ...
  if (needsInference.length > 0) {
    const inferSystem = `${CASTELLAN_IDENTITY}\n\nYour role: select objective gates for plan nodes.\n\n${GATE_LADDER_DOC}\n\n${gatePatternDoc()}\n\nSTRONGLY prefer selecting a pattern (free-form shell is flagged to the user). TRACTABILITY: a gate must be satisfiable by ONE cheap attempt from a fresh checkout using the AVAILABLE TOOLS in the survey. Do NOT emit a project-wide e2e SUITE ("npm run test:e2e"/"npm run e2e", a cypress run) or a project-wide npm script that may not exist. But a UI node's BEHAVIOR must be gated with TEETH, not a "grep for a class name" stub. For anything the user must SEE happen (a value updates on a click, a card flips on reveal, a control disables off-turn), PREFER a SINGLE-FILE PLAYWRIGHT behavioral spec: gate it as "npx playwright test <one .spec.ts>" and have the build write that one spec to drive the page and assert the [data-testid=...]/[data-action=...] behaviors the CONTRACT pins (the harness installs playwright + chromium on demand). ONE focused spec per node — never a whole suite. (The zero-dep "dom-behavior" pattern is an alternative when a browser is present but playwright is not.) Reserve "npm run build --if-present" + grep ONLY for "the page renders at all"; for logic/data use a unit test or node -e assertion. CONTRACT COHERENCE: a gate that sends requests or asserts responses MUST use the EXACT field names from the SHARED CONTRACT below — the build conforms to the contract, so a gate that invents a different field name (posts {url} when the contract says {longUrl}) makes the correct build fail. GATE STRENGTH (the build SEES this gate and builds to pass it, so a weak or wrong gate fails CORRECT code — get it right): (1) CONSUME every output, do NOT assert mere presence — never grep that a field merely EXISTS (a hardcoded constant passes that); instead USE the value in a later step whose success depends on it being correct: FOLLOW a returned short link and assert the redirect Location is the original URL; DECODE a returned QR and assert it encodes that link; READ BACK a created resource and assert the round-trip returns what you put in; do N actions and assert a counter equals N; CREATE then DELETE then assert the read is 404. (2) Only assert what the REQUEST supplied or the contract GUARANTEES on EVERY response: do NOT grep for an alias field on a request that supplied no alias (correct code omits it, so your gate fails it). Check a custom alias ONLY on the request that set one, and verify it by USING it (a GET on that alias redirects), not by grepping the response body. Prefer round-trip / conservation / delete-then-absent shapes over presence greps.${archetype === "web-app" ? ` ${WEB_APP_GATE_RULE}` : ""} Output ONLY JSON: {"gates":[{node,pattern,params} | {node,freeform}]}.`;
    // The gate author MUST see the shared contract: without it the gate invents its own field
    // names (it POSTed {"url":...} while the contract said {longUrl} → the built server, obeying the
    // contract, 400'd the gate's request → halt). Gates must assert against the SAME data shapes the
    // build conforms to, using the EXACT field names from the contract.
    const contractForGates = decomposed.contract.trim()
      ? `SHARED CONTRACT (your gates run against a build that conforms to THIS — use these EXACT field names in any request bodies and response assertions; do NOT rename url/longUrl, code/shortCode, etc.):\n${decomposed.contract.trim()}\n\n`
      : "";
    const inferUser = `${contractForGates}NODES:\n${needsInference.map((n) => `${n.id}: ${n.brief}`).join("\n")}\n\nREPOSITORY SURVEY (use REAL commands found here):\n${survey}`;
